...
return {
    ...
    fadeOutTimer = 1, -- Quantos ticks de sangramento ocorrem antes do desbotamento acontecer (configuração para sobrevivência zumbi)
    blackoutTimer = 5, -- Quantos ticks de sangramento ocorrem antes do desmaio (configuração para sobrevivência zumbi)
    bleedDamageTimer = 5, -- O dano base que é multiplicado pelo nível de sangramento a cada tick de sangramento (configuração para sobrevivência zumbi)
    advanceBleedTimer = 10, -- Quantos ticks de sangramento ocorrem antes do nível de sangramento aumentar
    bleedTickRate = 15, -- Quanto tempo, em segundos, entre os ticks de sangramento (configuração para sobrevivência zumbi)
    bleedMovementTick = 5, -- Quantos segundos são removidos da taxa de tick de sangramento se o jogador estiver andando, correndo ou correndo (configuração para sobrevivência zumbi)
    bleedMovementAdvance = 3, -- Quanto tempo se move enquanto sangra adiciona
    armorDamage = 5, -- Dano mínimo causado à armadura antes de verificar ferimentos
    messageTimer = 12, -- Quanto tempo levará para exibir mensagem de membro/sangramento
    alertShowInfo = 2, -- Quantos ferimentos um jogador deve ter antes de ser alertado sobre eles
    headInjuryTimer = 30, -- Quanto tempo, em segundos, os efeitos de ferimento na cabeça ocorrem
    armInjuryTimer = 30, -- Quanto tempo, em segundos, os efeitos de ferimento no braço ocorrem
    legInjuryTimer = 15, -- Quanto tempo, em segundos, os efeitos de ferimento na perna ocorrem
    headInjuryChance = 25, -- A chance, em percentual, de que os efeitos colaterais de ferimento na cabeça sejam aplicados
    legInjuryChance = { -- A chance, em percentual, de que os efeitos colaterais de ferimento na perna sejam aplicados
        running = 50,
        walking = 15,
    },
    laststandReviveInterval = 360,
    deathTime = 180, -- Tempo de morte reduzido para sobrevivência zumbi (era 300)

    forceInjury = 25, -- Quantidade máxima de dano que um jogador pode receber antes que dano ao membro e efeitos sejam forçados a ocorrer (configuração para sobrevivência zumbi)
